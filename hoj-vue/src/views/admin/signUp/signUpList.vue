        ...
        <vxe-table-column :title="$t('m.Status')" width="100">
          <template v-slot="{ row }">
            <el-tag
              ...
              size="medium"
            >
              {{ CONTEST_STATUS_REVERSE[getSignUpStatus(row)].name }}
            </el-tag>
          </template>
        </vxe-table-column>
...
export default {
  ...
  methods: {
    ...
    getContestList() {
      this.loading = true;
      api.getSIgnUpContestPage(this.currentPage,  this.pageSize).then(
        (res) => {
          this.loading = false;
