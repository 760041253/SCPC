<template>
  <div class="view">
    <el-card>
      ...
      <el-form :model="signUp" ref="signUp" label-position="top" :rules="rules">
        <el-row :gutter="20">
          <el-col :md="16" :xs="24" :span="12">
            <el-form-item :label="$t('m.SignUp_Title')" prop="title" required>
              <el-input v-model="signUp.title" :placeholder="$t('m.SignUp_Title')"></el-input>
            </el-form-item>
          </el-col>
          <el-col :md="8" :xs="24" :span="12">
            <el-form-item prop="contestId" :label="$t('m.SignUp_Contest')" required>
              <el-input v-model="signUp.contestId" :placeholder="$t('m.SignUp_Contest')"></el-input>
            </el-form-item>
          </el-col>
          <el-col :md="8" :xs="24">
            <el-form-item prop="startTime" :label="$t('m.SignUp_Start_Time')" required>
              <el-date-picker
                style="width: 100%;"
          ...
          <el-col :md="8" :xs="24">
            <el-form-item prop="endTime" :label="$t('m.SignUp_End_Time')" required>
              <el-date-picker
                style="width: 100%;"
          ...
          <el-col :md="8" :xs="24">
            <el-form-item :label="$t('m.SignUp_Duration')" required>
        ...
        <el-row>
          <el-col :md="8" :xs="24">
            <el-form-item prop="maxParticipants" :label="$t('m.Max_Participants')" required>
              <el-input-number
                style="width: 50%;"
                v-model="signUp.maxParticipants"
                :min="1"
                :max="3"
              ></el-input-number>
            </el-form-item>
        ...
        <el-row>
          <el-col :md="8" :xs="24">
            <el-form-item prop="contestType" :label="$t('m.Contest_Rule_Type')" required>
              <el-radio
                class="radio"
      ...
      <el-button type="primary" @click.native="saveSignUp">
        {{
        $t('m.Save')
        }}
      </el-button>
    ...
    return {
      ...
      rules: {
        title: [
          {
            required: true,
            message: this.$i18n.t("m.SignUp_Title_Check_Required"),
            trigger: "blur",
          },
        ],
        contestId: [
          {
            required: true,
            message: this.$i18n.t("m.SignUp_Contest_Check_Required"),
            trigger: "blur",
          },
        ],
        maxParticipants: [
          {
            required: true,
            message: this.$i18n.t("m.Max_Participants_Check_Required"),
            trigger: "blur",
          },
        ],
        startTime: [
          {
            required: true,
            message: this.$i18n.t("m.SignUp_Start_Time_Check_Required"),
            trigger: "blur",
          },
        ],
        endTime: [
          {
            required: true,
            message: this.$i18n.t("m.SignUp_End_Time_Check_Required"),
            trigger: "blur",
          },
        ],
        contestType: [
          {
            required: true,
            message: this.$i18n.t("m.ContestType_Check_Required"),
            trigger: "blur",
          },
        ],
      },
    ...
    saveSignUp() {
      if (this.$route.name === "admin-create-signUp") {
        if (!this.signUp.title) {
          myMessage.error(
            this.$i18n.t("m.SignUp_Title") + " " + this.$i18n.t("m.is_required")
          );
          return;
        }
        ...
        if (!this.signUp.contestType) {
          myMessage.error(this.$i18n.t("m.ContestType_Check_Required"));
          return;
        }
        api
          .admin_addSignUp(this.signUp)
          .then((res) => {
