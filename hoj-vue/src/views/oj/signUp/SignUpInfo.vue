  ...
  <div>
    <el-row v-if="isSignUp==false" type="flex" justify="space-around">
      <el-col :span="24">
        <el-card shadow>
          ...
          <el-form :model="signUpForm" ref="signUpForm" label-width="100px" :rules="rules">
    ...
    <div v-if="isSignUp==true" id="signUpInfo">
      <div v-for="(signUpItem, index) in signUpItems" :key="index">
        <div
          style="text-align: center;font-size: 1rem;color: rgb(147, 145, 145);letter-spacing: 5px;"
        >/ {{ $t('m.SignItem' ) }}{{ index+1 }} /</div>
        <br />
              ...
              <el-form :model="signUpItem" ref="signUpItem" label-width="100px" :rules="rules">
                ...
                <p id="teamInfo">
                  ...
                  <el-button
                    ...
                    size="small"
                    plain
                    @click="addMember_t(signUpItem.memberNum,index)"
                  >{{$t('m.AddMember')}}</el-button>
...
import { SIGN_UP_STATUS_REVERSE } from "@/common/constants";
export default {
  data() {
    return {
      isSignUp: false,
      ...
      rules: {
        teamName: [
          {
            required: true,
            message: this.$i18n.t("m.TeamName_Check_Required"),
            trigger: "blur",
          },
          {
            min: 1,
        ...
        englishTeamName: [
          {
            required: true,
            message: this.$i18n.t("m.English_TeamName_Check_Required"),
            trigger: "blur",
          },
          {
            min: 1,
            max: 25,
            message: "长度在 1 到 25 个英文字符",
            trigger: "blur",
          },
        ],
        teammatesName1: [
          {
            required: true,
            message: this.$i18n.t("m.TeammatesName_Check_Required"),
            trigger: "blur",
          },
          {
            pattern: /^[\u4e00-\u9fa5]+$/,
        ...
        studentClass1: [
          {
            required: true,
            message: this.$i18n.t("m.StudentClass_Check_Required"),
            trigger: "blur",
          },
        ...
        studentId1: [
          {
            required: true,
            message: this.$i18n.t("m.StudentId_Check_Required"),
            trigger: "blur",
          },
        ],
        usersName1: [
          {
            required: true,
            message: this.$i18n.t("m.UsersName_Check_Required"),
            trigger: "blur",
          },
        ],
        phoneNumber1: [
        ...
        size1: [
          {
            required: true,
            message: this.$i18n.t("m.Size_Check_Required"),
            trigger: "blur",
          },
        ...
        gender1: [
          {
            required: true,
            message: this.$i18n.t("m.Gender_Check_Required"),
            trigger: "blur",
          },
        ],
        teammatesName2: [
  ...
  methods: {
    handleSignUp() {
      ...
      if (this.memberNum == 1) {
        ...
        if (
          !this.signUpForm.phoneNumber1 &&
          this.signUpForm.phoneNumber1.length < 11
        ) {
      ...
      this.teammatesName =
        this.signUpForm.teammatesName1 +
        "-" +
      ...
      this.$refs["signUpForm"].validate((valid) => {
        if (valid) {
          ...
          if (this.signStatus == 200) {
            ...
            api.applyForUpdateTeamInfo(formData).then((res) => {
            ...
            if (this.signStatus === "") {
              ...
              formData.id = this.teamId;
              api.putTeamInfo(formData).then((res) => {
                if (res.data.status == 200) {
        ...
        if (res.data.status == 200) {
          if (res.data.data != "") {
            ...
            for (let i in listData) {
              ...
              signUpForm["teammatesName1"] =
                listData[i].teammatesName.split("-")[0];
              signUpForm["teammatesName2"] =
                listData[i].teammatesName.split("-")[1] == "*"
                  ? ""
                  : listData[i].teammatesName.split("-")[1];
              signUpForm["teammatesName3"] =
                listData[i].teammatesName.split("-")[2] == "*"
                  ? ""
                  : listData[i].teammatesName.split("-")[2];
              signUpForm["studentClass1"] =
                listData[i].studentClass.split("-")[0];
              signUpForm["studentClass2"] =
                listData[i].studentClass.split("-")[1] == "*"
                  ? ""
                  : listData[i].studentClass.split("-")[1];
              signUpForm["studentClass3"] =
                listData[i].studentClass.split("-")[2] == "*"
                  ? ""
                  : listData[i].studentClass.split("-")[2];
              signUpForm["studentId1"] = listData[i].studentId.split("-")[0];
              signUpForm["studentId2"] =
                listData[i].studentId.split("-")[1] == "*"
                  ? ""
                  : listData[i].studentId.split("-")[1];
              signUpForm["studentId3"] =
                listData[i].studentId.split("-")[2] == "*"
                  ? ""
                  : listData[i].studentId.split("-")[2];
              signUpForm["nickname1"] =
                listData[i].nickname.split("-")[0] == "*"
                  ? ""
                  : listData[i].nickname.split("-")[0];
              signUpForm["nickname2"] =
                listData[i].nickname.split("-")[1] == "*"
                  ? ""
                  : listData[i].nickname.split("-")[1];
              signUpForm["nickname3"] =
                listData[i].nickname.split("-")[2] == "*"
                  ? ""
                  : listData[i].nickname.split("-")[2];
              signUpForm["usersName1"] = listData[i].usersName.split("-")[0];
              signUpForm["usersName2"] =
                listData[i].usersName.split("-")[1] == "*"
                  ? ""
                  : listData[i].usersName.split("-")[1];
              signUpForm["usersName3"] =
                listData[i].usersName.split("-")[2] == "*"
                  ? ""
                  : listData[i].usersName.split("-")[2];
              signUpForm["phoneNumber1"] =
                listData[i].phoneNumber.split("-")[0];
              signUpForm["phoneNumber2"] =
                listData[i].phoneNumber.split("-")[1] == "*"
                  ? ""
                  : listData[i].phoneNumber.split("-")[1];
              signUpForm["phoneNumber3"] =
                listData[i].phoneNumber.split("-")[2] == "*"
                  ? ""
                  : listData[i].phoneNumber.split("-")[2];
              signUpForm["size1"] = listData[i].size.split("-")[0];
              signUpForm["size2"] =
                listData[i].size.split("-")[1] == "*"
                  ? ""
                  : listData[i].size.split("-")[1];
              signUpForm["size3"] =
                listData[i].size.split("-")[2] == "*"
                  ? ""
                  : listData[i].size.split("-")[2];
              signUpForm["gender1"] = listData[i].gender.split("-")[0];
              signUpForm["gender2"] =
                listData[i].gender.split("-")[1] == "*"
                  ? ""
                  : listData[i].gender.split("-")[1];
              signUpForm["gender3"] =
                listData[i].gender.split("-")[2] == "*"
                  ? ""
                  : listData[i].gender.split("-")[2];
              if (
                signUpForm["teammatesName2"] != "" &&
